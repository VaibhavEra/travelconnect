...
### Issue #25 - Post-pickup trip cancellation — cancel button and sender OTP visibility ✅

**Type:** Frontend Bug Fix — UI Visibility  
**Priority:** HIGH  
**Time:** 30–45 minutes  
**Date:** 2026-02-18
...
Two UI gaps prevented the post-pickup cancellation flow from working
end-to-end despite the backend being fully functional:

1. `canCancel` in `my-trips/[id].tsx` only included `upcoming` and
   `locked` statuses. Any trip with a `picked_up` parcel will be
   `in_progress` (departure time has passed), so the Cancel Trip
   button was permanently hidden in the exact scenario where OTP
   cancellation is needed.

2. `my-requests/[id].tsx` had no Cancellation OTP card. The sender
   had no way to see the OTP generated by the traveller — it only
   existed in the database. The existing pickup and delivery OTP
   cards were present, but `cancellation_otp` was never rendered.

**What Was Already Working:**

- `generateCancellationOtp` → OTP stored in DB ✅
- `CancellationOtpModal` on traveller side → OTP entry + verification ✅
- `verifyCancellationOtpAndCancel` → cancels trip + request ✅
- Full flow verified working on `locked` trips before this fix ✅

**Frontend Changes (Applied via GitHub PR - 2026-02-18):**

1. **`app/(tabs)/my-trips/[id].tsx`** (1-line fix)
   - **Updated:** `canCancel` to include `in_progress` status
   - **Before:** `status === "upcoming" || status === "locked"`
   - **After:** `status === "upcoming" || status === "locked" || status === "in_progress"`

2. **`app/my-requests/[id].tsx`** (New OTP card)
   - **Added:** `cancellationExpiryText` variable alongside existing
     `pickupExpiryText` / `deliveryExpiryText` for consistency
   - **Added:** Cancellation OTP card rendered when:
     - `status === "picked_up"`
     - `cancellation_otp` is non-null
     - `cancellation_otp_expiry > new Date()` (not yet expired)
   - Card shows: 6-digit OTP, expiry countdown, helper text
     ("Share only if you agree to cancellation"), and an
     irreversibility warning
   - No new styles introduced — reuses `otpRow`, `otpCode`,
     `helperText`, `regenerateButton` from existing stylesheet

**Testing:**

- ✅ `locked` trip with `picked_up` parcel → Cancel button visible, full OTP flow completes (was already working)
- ✅ `in_progress` trip with `picked_up` parcel → Cancel button now visible
- ✅ Sender opens `my-requests/[id].tsx` → Cancellation OTP card appears with code + countdown
- ✅ Traveller enters OTP → trip + request cancelled → card disappears from sender screen
- ✅ Expired OTP (`cancellation_otp_expiry < now`) → card does not render
- ✅ No OTP generated yet (`cancellation_otp` is null) → card does not render
- ✅ Pickup and Delivery OTP cards unaffected

**Acceptance Criteria:**

- ✅ Cancel Trip button visible for `in_progress` trips
- ✅ Sender sees cancellation OTP when traveller requests post-pickup cancellation
- ✅ OTP card only shows when OTP exists and has not expired
- ✅ No new styles or components introduced

**Frontend Changes:**

- `app/(tabs)/my-trips/[id].tsx` — Extended `canCancel` to include `in_progress`
- `app/my-requests/[id].tsx` — Added `cancellationExpiryText` + Cancellation OTP card

**Type Updates:**

- None — `cancellation_otp` and `cancellation_otp_expiry` already
  present in the request type via `select('*')`
...
- Depends on: `generate_cancellation_otp` and
  `verify_cancellation_otp_and_cancel_trip` backend functions ✅
- Complements: Pickup OTP flow (Issue #21) and Delivery OTP flow
...
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------- |
| `stores/tripStore.ts`                                     | Removed `notes` from `Trip` type, `EditableGeneralTripFields`, `createTrip()`, `normalizeTrip()`, `updateTripGeneralFields()` whitelist; added `updateTripDates()` method; updated permission logic                                                                                                                                                                                                                                                                                        | #4, #7, #27                  |
| `stores/requestStore.ts`                                  | Added `completedRequests` state + `getCompletedRequests()` action; scoped `getIncomingRequests` and `getAcceptedRequests` queries; updated `updateRequestStatus`; added `canEditRequestDetails()`, `canEditReceiverDetails()`, `updateReceiverDetails()`; added `regeneratePickupOtp()`, `regenerateDeliveryOtp()`; updated `PickupOtpVerificationResult` and `DeliveryOtpVerificationResult` types; updated `verifyPickupOtp()` and `verifyDeliveryOtp()` to use structured JSON response | #17, #18, #19, #20, #21, #42 |
| `lib/validations/trip.ts`                                 | Added `parseDateTime()` helper; removed `notes` field; fixed departure ≥ now+1hr, arrival > departure, same-date check; updated error paths to time fields                                                                                                                                                                                                                                                                                                                                 | #5, #6, #7                   |
| `lib/validations/trip-edit.ts`                            | Removed `notes` from `tripDetailsSchema`; removed deprecated backward-compatibility exports (`tripDetailsSchema`, `TripDetailsFormData`, `tripDatesSchema`, `TripDatesFormData`); verified `parseDateTime` usage and proper validation rules                                                                                                                                                                                                                                               | #7, #15                      |
| `lib/validations/request-edit.ts` _(new)_                 | Created `requestDetailsEditSchema` (description, category, photos) and `receiverDetailsEditSchema` (contact name, phone)                                                                                                                                                                                                                                                                                                                                                                   | #19                          |
| `lib/constants/filters.ts`                                | Added `expired`, `locked`, `in_progress` to `TRIP_FILTERS`; expanded `REQUEST_FILTERS` to all 8 statuses; added `INCOMING_REQUEST_FILTERS`, `ACTIVE_REQUEST_FILTERS`, `COMPLETED_REQUEST_FILTERS` with key types `IncomingRequestFilterKey`, `ActiveRequestFilterKey`, `CompletedRequestFilterKey`                                                                                                                                                                                         | #10, #17, #21                |
| `lib/utils/fileUpload.ts`                                 | Added `uploadFile()` wrapper (accepts local URI + bucket → returns public URL)                                                                                                                                                                                                                                                                                                                                                                                                             | #8                           |
| `app/create-trip.tsx`                                     | Removed `notes` from form defaults and reset; added `uploadFile` import; added `isSubmitting` state; updated `onSubmit()` to upload ticket before calling `createTrip()`; simplified `getErrorMessage()` (59→26 lines); fixed arrival date picker `minimumDate` fallback to `new Date()`                                                                                                                                                                                                   | #6, #7, #8                   |
| `app/explore/request-form.tsx`                            | Added `uploadFile` import; added `isSubmitting` state; updated `onSubmit()` to upload parcel photos in parallel via `Promise.all()`                                                                                                                                                                                                                                                                                                                                                        | #8, #9                       |
| `app/(tabs)/my-trips/index.tsx`                           | Removed local `TripFilter` type and `FILTER_CONFIG`; imported `TRIP_FILTERS` and `TripFilterKey` from `filters.ts`; updated `getFilterCount()` signature                                                                                                                                                                                                                                                                                                                                   | #10                          |
| `app/(tabs)/my-requests/index.tsx`                        | Removed local `StatusFilter` type and custom `FILTER_CONFIG`; imported `REQUEST_FILTERS` and `RequestFilterKey`; simplified filter logic to 1:1 status matching                                                                                                                                                                                                                                                                                                                            | #17                          |
| `app/(tabs)/requests/index.tsx`                           | Replaced 2-view switcher (Incoming/Active) with 3-tab layout (Incoming/Active/Completed); removed local type aliases; imported shared filter constants; added `completedFilter` state; added `getCompletedRequests` call; added brute-force props (`selectedFailedAttempts`, `selectedBlockedUntil`) passed to `VerifyOtpModal` on open                                                                                                                                                    | #20, #21                     |
| `app/(tabs)/my-trips/[id].tsx`                            | Removed global header edit button; added `canEditDetails` / `canEditDates` permission state via `useEffect`; added per-section conditional edit buttons; route and transport fields now read-only                                                                                                                                                                                                                                                                                          | #11                          |
| `app/my-requests/[id].tsx`                                | Major refactor (~180 lines removed); added permission state management; added traveller info card, PNR/ticket display, Pickup OTP card, Delivery OTP card with regenerate buttons; removed inline cancel modal; added conditional edit buttons per section                                                                                                                                                                                                                                 | #19, #42                     |
| `app/requests/[id].tsx`                                   | Replaced multi-card layout with single unified `mainCard`; moved action buttons into `ScrollView`; removed `borderBottomWidth` from header; added status pill banner inside card; matched layout pixel-for-pixel with `trip-preview.tsx`                                                                                                                                                                                                                                                   | #22                          |
| `components/forms/FileUploadButton.tsx`                   | Removed immediate upload on selection; stores local `file://` URI in state; removed `userId` prop; renamed button to "Select Ticket"                                                                                                                                                                                                                                                                                                                                                       | #8                           |
| `components/forms/ImagePicker.tsx`                        | Removed all Supabase upload logic and `uploadImage()` function; stores array of local `file://` URIs; added `isSelecting` state                                                                                                                                                                                                                                                                                                                                                            | #8, #9                       |
| `components/trip/EditTripDetailsModal.tsx`                | Removed `notes` controller block, `defaultValues`, and `updateTrip()` call for notes                                                                                                                                                                                                                                                                                                                                                                                                       | #7, #11                      |
| `components/trip/EditTripDatesModal.tsx`                  | Verification only — already updated in Issue #7; no additional changes                                                                                                                                                                                                                                                                                                                                                                                                                     | #14                          |
| `components/request/CancelRequestModal.tsx` _(new)_       | Extracted from inline implementation; accepts `requestStatus` prop; removed 24h restriction logic; conditional rendering for pending/accepted                                                                                                                                                                                                                                                                                                                                              | #19                          |
| `components/request/EditRequestDetailsModal.tsx` _(new)_  | Edit item description, category, and parcel photos; validates against trip's allowed categories; backend permission check (pending only); react-hook-form + Zod                                                                                                                                                                                                                                                                                                                            | #19                          |
| `components/request/EditReceiverDetailsModal.tsx` _(new)_ | Edit delivery contact name and phone; allows edits until delivered; backend validation via `can_edit_receiver_details` RPC                                                                                                                                                                                                                                                                                                                                                                 | #19                          |
| `components/request/RequestCard.tsx`                      | Complete redesign — replaced two-section layout with TripCard-style layout; added status banner, city columns, transport icon with connecting lines, animated press effect; fixed type-safe transport icon handling                                                                                                                                                                                                                                                                        | #18                          |
| `components/modals/VerifyOtpModal.tsx`                    | Replaced brittle substring matching with error code matching (`blocked`, `expired`, `invalid_otp`); removed stray `console.error`                                                                                                                                                                                                                                                                                                                                                          | #20                          |
| `types/database.types.ts`                                 | Regenerated — removed `notes` from `trips` row type; added new RPC function types                                                                                                                                                                                                                                                                                                                                                                                                          | #7, #19                      |

---

#### Updated: Backend Changed

All changes applied **manually via Supabase SQL Editor** (no migration files).

##### Functions

| Function                                      | Action                                                                                                                                                                                                                                             | Issue | Date       |
| --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | ---------- |
| `create_trip_with_validation()`               | Updated — removed `p_notes` parameter and `notes` INSERT; legacy overload with `p_notes` dropped                                                                                                                                                   | #5    | 2026-02-10 |
| `can_edit_trip()`                             | Updated — removed 24h buffer; now only checks pickup status                                                                                                                                                                                        | #4    | 2026-02-08 |
| `can_edit_trip_dates()`                       | Updated — delegates to `can_edit_trip()` for consistency                                                                                                                                                                                           | #4    | 2026-02-08 |
| `cancel_request_with_validation()`            | Updated — removed 24h time restriction; simplified status check to block only `picked_up` directly                                                                                                                                                 | #2    | 2026-02-09 |
| `validate_status_transition()`                | Updated — added cascade cancellation logic for `picked_up` requests when trip is being cancelled                                                                                                                                                   | #2    | 2026-02-09 |
| `update_trip_status_from_requests()`          | Updated — removed logic that moved trip to `in_progress` on pickup; kept auto-complete and auto-expire logic                                                                                                                                       | #1    | 2026-02-09 |
| `expire_old_requests()`                       | Enhanced — added expiry for `in_progress` trips past arrival with no active requests                                                                                                                                                               | #1    | 2026-02-09 |
| `expire_pending_requests_on_trip_start_row()` | Created — row-level trigger function; when trip moves to `in_progress`, all `pending` requests → `expired`                                                                                                                                         | #3    | 2026-02-09 |
| `user_can_view_trip(uuid)`                    | Created — `SECURITY DEFINER` / `STABLE` function to break RLS infinite recursion; checks if user has any request for a given `trip_id`                                                                                                             | #18   | 2026-02-13 |
| `verify_pickup_otp()`                         | Redesigned body (same signature) — replaced `RAISE EXCEPTION` pattern with structured JSON returns; `RETURNING` clause captures post-increment attempt counts; all failure paths (`blocked`, `expired`, `invalid_otp`) return without rolling back | #20   | 2026-02-18 |
| `verify_delivery_otp()`                       | Same redesign as `verify_pickup_otp()` mirrored for delivery                                                                                                                                                                                       | #20   | 2026-02-18 |
| `clear_otp_after_use()`                       | Body updated — clears OTP on status transition; signature unchanged                                                                                                                                                                                | #24   | 2026-02-18 |
| `can_edit_request_details(request_id UUID)`   | Already existed — returns boolean; pending status only                                                                                                                                                                                             | #19   | —          |
| `can_edit_receiver_details(request_id UUID)`  | Already existed — returns boolean; before delivery                                                                                                                                                                                                 | #19   | —          |
| `update_request_details(...)`                 | Already existed — updates with validation                                                                                                                                                                                                          | #19   | —          |
| `update_receiver_details(...)`                | Already existed — updates contact info                                                                                                                                                                                                             | #19   | —          |
| `regenerate_pickup_otp(p_request_id uuid)`    | Already existed — returns new OTP text                                                                                                                                                                                                             | #42   | —          |
| `regenerate_delivery_otp(p_request_id uuid)`  | Already existed — returns new OTP text                                                                                                                                                                                                             | #42   | —          |

##### Tables

| Table   | Action                 | Issue | Date       |
| ------- | ---------------------- | ----- | ---------- |
| `trips` | Dropped `notes` column | #5    | 2026-02-10 |

##### Triggers

| Trigger                                | Action                                                                                                                                                        | Target Table      | Issue | Date       |
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- | ----- | ---------- |
| `expire_pending_on_trip_start_trigger` | Created — `AFTER UPDATE`, fires when `NEW.status = 'in_progress' AND OLD.status != 'in_progress'`; calls `expire_pending_requests_on_trip_start_row()`        | `trips`           | #3    | 2026-02-09 |
| `unlock_trip_on_all_cancellations`     | Created — automatically unlocks trip from `locked` → `upcoming` when all accepted/picked_up requests are cancelled/rejected (only if departure hasn't passed) | `parcel_requests` | #1    | 2026-02-09 |
| `prevent_cancellation_with_pickups`    | **Dropped** — was blocking OTP-based emergency cancellations                                                                                                  | `trips`           | #2    | 2026-02-09 |
| `trigger_auto_complete_trip`           | **Dropped** — duplicate logic                                                                                                                                 | `trips`           | #1    | 2026-02-09 |
| `update_trip_status_trigger`           | **Dropped** — duplicate trigger                                                                                                                               | `trips`           | #1    | 2026-02-09 |

##### RLS Policies

| Policy                                              | Table   | Action                                                                                                                 | Issue | Date       |
| --------------------------------------------------- | ------- | ---------------------------------------------------------------------------------------------------------------------- | ----- | ---------- |
| `traveller_update_general_fields_before_acceptance` | `trips` | Added                                                                                                                  | #4    | 2026-02-08 |
| `traveller_update_dates_before_pickup`              | `trips` | Added                                                                                                                  | #4    | 2026-02-08 |
| `"Users can view trips"`                            | `trips` | Updated — replaced direct subquery (caused RLS recursion) with `user_can_view_trip(id)` SECURITY DEFINER function call | #18   | 2026-02-13 |

##### Cron Jobs

| Job                               | Action                                                                               | Schedule                    | Issue | Date       |
| --------------------------------- | ------------------------------------------------------------------------------------ | --------------------------- | ----- | ---------- |
| `transition-trips-to-in-progress` | Created — moves `upcoming`/`locked` trips → `in_progress` when departure time passes | Every 5 min (`*/5 * * * *`) | #1    | 2026-02-09 |
| `auto-transition-trips`           | **Removed** — replaced by above (was running every minute)                           | —                           | #1    | 2026-02-09 |
